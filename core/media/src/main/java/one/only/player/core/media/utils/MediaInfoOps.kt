...
object MediaInfoOps {
        ...
        runCatching {
            try {
                ...
                try {
                    ...
                    // Fix fdsan crash: detach FD so libmediainfo can close it without fdsan aborting
                    val fd = pfd.detachFd()
                    try {
                        mi.Open(fd, fileName)
                    } finally {
                        // We don't call pfd.close() because detachFd() already invalidated it
                        // libmediainfo Close() or internal logic will close the raw FD
                    }
                ...
                Logger.error("MediaInfoOps", "MediaInfo native library not found", e)
                fetchMetadataFallback(context, uri).getOrThrow()
            } catch (e: Exception) {
                Logger.error("MediaInfoOps", "MediaInfo failed, using fallback", e)
                fetchMetadataFallback(context, uri).getOrThrow()
            }
        }
    }
            ...
            try {
                ...
                try {
                    ...
                    buildString {
                        ...
                        appendLine()
                        appendLine("=".repeat(60))
                        appendLine("Generated by OnlyPlayer using MediaInfoLib")
                        appendLine("=".repeat(60))
                    }
                } finally {
                    // pfd.close() is redundant after detachFd()
                }
            } finally {
                mi.Close()
            }
    ...
    private fun extractVideoStreams(mi: MediaInfo): List<VideoStreamInfo> {
        ...
        return (0 until count).map { i ->
            VideoStreamInfo(
                ...
                title = mi.getInfo(MediaInfo.Stream.Video, i, "Title"),
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        // pfd.close() not needed
                    }
                } finally {
                    mi.Close()
                }
            } catch (e: Exception) {
                fetchBasicMetadataFallback(context, uri).getOrThrow()
            }
        }
    }

    private suspend fun fetchBasicMetadataFallback(
        context: Context,
        uri: Uri,
    ): Result<VideoMetadata> = withContext(Dispatchers.IO) {
        runCatching {
            ...
            try {
                ...
                VideoMetadata(
                    sizeBytes = 0L, // Retriever doesn't easily give file size from Uri without more work
                    durationMs = duration,
                    width = width,
                    height = height,
                    fps = 0f,
                    hasEmbeddedSubtitles = false, // Hard to tell with Retriever
                )
            } finally {
                retriever.release()
            }
    ...
    private suspend fun fetchMetadataFallback(
        context: Context,
        uri: Uri,
    ): Result<MediaInfoData> = withContext(Dispatchers.IO) {
        runCatching {
