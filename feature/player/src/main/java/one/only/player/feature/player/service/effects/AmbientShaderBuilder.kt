...
vec4 HOOKED_tex(vec2 uv) {
    return texture2D(uTexSampler, clamp(uv, vec2(0.0), vec2(1.0)));
}

#define HOOKED_pos vTexSamplingCoord
...
void main() {
    ...
    if (video_uv.x >= 0.0 && video_uv.x <= 1.0 &&
        ...
        gl_FragColor = HOOKED_tex(video_uv);
        return;
    }
...
vec4 trace_anchor_strip(vec2 anchor_uv, vec3 anchor_edge, vec2 inward_dir, vec2 ortho_dir, float outside_norm) {
    ...
    gl_FragColor = vec4(extend_rgb, coherence);
    return;
}
...
vec4 sample_predictive_fill(vec2 edge_origin, vec3 edge_rgb, vec2 inward_dir, vec2 ortho_dir, float outside_norm) {
    ...
    gl_FragColor = vec4(acc / max(acc_weight, 1e-5), confidence_acc / max(acc_weight, 1e-5));
    return;
}
