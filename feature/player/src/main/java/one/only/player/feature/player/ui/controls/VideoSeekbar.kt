...
fun SeekbarWithTimers(
  ...
  paused: Boolean,
  seekbarStyle: one.only.player.core.model.SeekbarStyle = one.only.player.core.model.SeekbarStyle.Wavy,
...
fun VideoSeekbar(
  ...
  onValueChange: (Float) -> Unit,
  onValueChangeFinished: (Float) -> Unit,
  chapters: List<Segment> = emptyList(),
...
fun SeekbarContent(
  ...
  seekbarStyle: one.only.player.core.model.SeekbarStyle,
  loopStart: Float?,
  loopEnd: Float?,
  ...
  val overlayTrackHeight =
    when (seekbarStyle) {
      one.only.player.core.model.SeekbarStyle.Slim ->
        when {
          isUserInteracting -> 15.dp
          paused -> 6.dp
          else -> 8.dp
        }
      one.only.player.core.model.SeekbarStyle.Thick -> 16.dp
      one.only.player.core.model.SeekbarStyle.Standard -> 8.dp
      one.only.player.core.model.SeekbarStyle.Wavy -> 8.dp
    }
  ...
  Box(
    ...
    if (useSeekPreview && videoUri != null) {
      SeekThumbnailPreviewBubble(
        ...
        modifier = Modifier.align(Alignment.BottomStart).offset(y = (-touchAreaHeight) + 8.dp)
      )
    }
    ...
    Box(
      ...
      Box(
        ...
        when (seekbarStyle) {
          one.only.player.core.model.SeekbarStyle.Standard -> {
            StandardSeekbar(
              position = position,
              duration = duration,
              chapters = chapters,
              ...
              seekbarStyle = one.only.player.core.model.SeekbarStyle.Standard,
              onSeek = { newPosition ->
                onUserInteractionChange(true)
          ...
          one.only.player.core.model.SeekbarStyle.Wavy -> {
            SquigglySeekbar(
              position = position,
              duration = duration,
              chapters = chapters,
              ...
              isScrubbing = isUserInteracting,
              useWavySeekbar = true,
              seekbarStyle = one.only.player.core.model.SeekbarStyle.Wavy,
          ...
          one.only.player.core.model.SeekbarStyle.Thick -> {
            StandardSeekbar(
              position = position,
              duration = duration,
              chapters = chapters,
              isPaused = paused,
              isScrubbing = isUserInteracting,
              seekbarStyle = one.only.player.core.model.SeekbarStyle.Thick,
              onSeek = { newPosition ->
                onUserInteractionChange(true)
                latestInteractionPosition = newPosition.coerceIn(0f, duration)
                onUserPositionChange(newPosition)
      ...
      Column(
        ...
        Surface(
          ...
          Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
            if (videoUri != null) {
              val snappedPositionMs = (position.toLong() / 1000) * 1000
              val lastBitmap = remember { mutableStateOf<Bitmap?>(null) }
              SubcomposeAsyncImage(
                model = ImageRequest.Builder(LocalContext.current)
                  .data(videoUri)
                  .decoderFactory(VideoFrameDecoder.Factory())
                  .videoFrameMillis(snappedPositionMs)
                  .build(),
                contentDescription = null,
                contentScale = ContentScale.Crop,
                modifier = Modifier.fillMaxSize(),
              ) {
                val currentState = painter.state.collectAsState().value
                when (currentState) {
                  is AsyncImagePainter.State.Success -> {
                    val image = currentState.result.image
                    if (image is coil3.BitmapImage) {
                      lastBitmap.value = image.bitmap
                    }
                    Image(
        ...
        Surface(
          ...
          Text(
            text = one.only.player.core.common.Utils.formatDurationMillisSign(position.toInt().toLong()),
...
fun SeekbarStylePreview(
    ...
    if (style == one.only.player.core.model.SeekbarStyle.Wavy) {
        SquigglySeekbar(
...
fun VideoTimer(
  ...
  Text(
    ...
    text = one.only.player.core.common.Utils.formatDurationMillisSign(value.toInt().toLong()),
    color = Color.White,
...
fun StandardSeekbar(
    ...
    seekbarStyle: one.only.player.core.model.SeekbarStyle = one.only.player.core.model.SeekbarStyle.Standard,
    onSeek: (Float) -> Unit,
    onSeekFinished: () -> Unit,
    loopStart: Float? = null,
    loopEnd: Float? = null,
    ...
    val isThick = seekbarStyle == one.only.player.core.model.SeekbarStyle.Thick
...
val androidx.compose.material3.MaterialTheme.spacing get() = Spacing()
class Spacing(val large: androidx.compose.ui.unit.Dp = 16.dp)

object AppMotion {
    object Spatial {
        object Expressive { val dampingRatio = 0.85f; val stiffness = 300f }
        object Standard { val dampingRatio = 0.85f; val stiffness = 300f }
    }
    object Effect {
        object Alpha { val dampingRatio = 1f; val stiffness = 100f }
    }
}

data class Segment(val start: Float)
data class SkipSegment(val startSeconds: Float, val endSeconds: Float, val type: SkipSegmentType)
enum class SkipSegmentType { OPENING, ENDING, RECAP, MIXED_ED; val accentColor = androidx.compose.ui.graphics.Color.Red }
